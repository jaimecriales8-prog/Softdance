...
type Alumna = { id: string; nombre: string }
type Familia = { id: string; nombre: string; email: string; telefono: string | null; alumnas: Alumna[] }
...
export default function ReciboClient({ familia, escuela, mensualidades, eventos }: {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6 print:hidden">
        <div>
          <h1 className="text-2xl font-bold text-white">Recibo — {familia.nombre}</h1>
          <p className="text-white/40 text-sm mt-0.5">Estado de cuenta completo</p>
        </div>
        <div className="flex gap-3">
          <a href={`/escuela/familias/${familia.id}`}
            className="border border-white/10 text-white/60 text-sm px-4 py-2 rounded-lg hover:bg-white/5 transition-colors">
            ← Volver
          </a>
      ...
      <div className="max-w-3xl mx-auto print:max-w-none print:mx-0">
        ...
        <div className="hidden print:block mb-8 border-b border-black/20 pb-4">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right text-sm text-gray-500">
              <p>{new Date().toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
            </div>
          </div>
          <div className="mt-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 print:grid-cols-4 print:gap-2">
          {[
            { label: 'Mensualidades pendientes', value: totalPendienteMens, color: 'text-yellow-400 print:text-yellow-600' },
            { label: 'Mensualidades pagadas', value: totalPagadoMens, color: 'text-green-400 print:text-green-700' },
            { label: 'Eventos pendientes', value: totalPendienteEv, color: 'text-yellow-400 print:text-yellow-600' },
            { label: 'Eventos pagados', value: totalPagadoEv, color: 'text-green-400 print:text-green-700' },
          ].map(s => (
            <div key={s.label} className="bg-white/5 border border-white/10 rounded-xl p-4 print:bg-gray-50 print:border-gray-200 print:rounded print:p-3">
              ...
              <p className={`text-lg font-bold ${s.color}`}>${s.value.toLocaleString('es-CO')}</p>
            </div>
          ))}
        </div>
        ...
        <section className="mb-8">
          <h2 className="text-base font-semibold text-white mb-3 print:text-black print:text-lg print:mb-2">
            Mensualidades
          </h2>
          {mensualidades.length === 0 ? (
            <p className="text-white/30 text-sm print:text-gray-400">Sin mensualidades registradas</p>
          ) : (
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden print:bg-white print:border-gray-200 print:rounded">
                  ...
                  {mensualidades.map((m, i) => (
                    <tr key={m.id} className={`${i < mensualidades.length - 1 ? 'border-b border-white/5 print:border-gray-100' : ''}`}>
                      <td className="px-4 py-3">
                        ...
                        {m.fecha_limite && (
                          <p className="text-white/40 text-xs print:text-gray-400">Vence {fmtFecha(m.fecha_limite)}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
        ...
        <section>
          ...
          {eventos.length === 0 ? (
            <p className="text-white/30 text-sm print:text-gray-400">Sin eventos registrados</p>
          ) : (
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden print:bg-white print:border-gray-200 print:rounded">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 print:border-gray-200 print:bg-gray-50">
                    ...
                    <th className="text-left text-white/40 text-xs uppercase tracking-wider px-4 py-2.5 print:text-gray-500">Cuotas</th>
                    <th className="text-right text-white/40 text-xs uppercase tracking-wider px-4 py-2.5 print:text-gray-500">Total</th>
                    <th className="text-right text-white/40 text-xs uppercase tracking-wider px-4 py-2.5 print:text-gray-500">Estado</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={ev.id} className={`${i < eventos.length - 1 ? 'border-b border-white/5 print:border-gray-100' : ''}`}>
                        <td className="px-4 py-3">
                          ...
                          {(ev.lineas ?? []).filter(l => l.valor > 0).map((l: any, li: number) => (
                            <p key={li} className="text-white/30 text-xs print:text-gray-400">{l.concepto} — ${l.valor.toLocaleString('es-CO')}</p>
                          ))}
                        </td>
                        <td className="px-4 py-3 text-white/60 print:text-gray-600">{ev.alumnas.nombre}</td>
                        <td className="px-4 py-3">
                          {ev.eventos.num_cuotas === 1 ? (
                            <span className="text-white/40 text-xs print:text-gray-400">Pago único</span>
                          ) : (
                            <div className="space-y-0.5">
                              {cuotas.map(c => (
                                <p key={c.numero} className={`text-xs ${c.estado === 'pagado' ? 'text-green-400 print:text-green-700' : 'text-white/40 print:text-gray-400'}`}>
                                  Cuota {c.numero} · ${Math.round(montoCuota).toLocaleString('es-CO')} · {c.estado === 'pagado' ? '✓ Pagado' : 'Pendiente'}
                                </p>
                              ))}
                            </div>
                          )}
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ev.estado === 'pagado' ? 'bg-green-500/15 text-green-400 print:bg-green-100 print:text-green-700' : 'bg-yellow-500/15 text-yellow-400 print:bg-yellow-100 print:text-yellow-700'}`}>
                            {ev.estado === 'pagado' ? 'Pagado' : pagadas > 0 ? `${pagadas}/${ev.eventos.num_cuotas}` : 'Pendiente'}
                          </span>
                        </td>
                      </tr>
                    )
        ...
        <div className="hidden print:block mt-8 pt-4 border-t border-gray-200 text-xs text-gray-400 text-center">
          {escuela.nombre} · Documento generado el {new Date().toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}
        </div>
      </div>
    </div>
  )
