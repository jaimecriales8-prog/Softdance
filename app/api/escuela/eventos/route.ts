...
export async function GET(request: NextRequest) {
  ...
  const { data } = await supabase
    .from('eventos')
    .select('*, evento_alumna(id, estado, alumnas(id, nombre, familias(nombre)))')
...
export async function POST(request: NextRequest) {
  ...
  const { nombre, descripcion, precio, fecha } = await request.json()
  if (!nombre) return NextResponse.json({ error: 'Falta nombre' }, { status: 400 })

  const { data, error } = await supabase.from('eventos')
    .insert({ escuela_id: escuelaId, nombre, descripcion: descripcion || null, precio: precio || 0, fecha: fecha || null })
    .select('*, evento_alumna(id, estado, alumnas(id, nombre, familias(nombre)))')
    .single()
...
export async function PATCH(request: NextRequest) {
  ...
  const { id, nombre, descripcion, precio, fecha, activo } = await request.json()
  if (!id) return NextResponse.json({ error: 'Falta id' }, { status: 400 })

  const { data, error } = await supabase.from('eventos')
    .update({ nombre, descripcion: descripcion || null, precio, fecha: fecha || null, activo })
